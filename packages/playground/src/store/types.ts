import { ComputedRef, DebuggerEvent, Ref, UnwrapRef, WatchOptions } from 'vue-demi'
    ...
    after: (
        callback: A extends Record<ActionName, _Method>
            ? (
              ...
              ) => void | ReturnType<A[ActionName]> | _Awaited<ReturnType<A[ActionName]>>
            : () => void
    ) => void
...
> extends DefineStoreOptionsBase<S, Store<Id, S, G, A>> {
     ...
     */
    actions?: A &
        ThisType<A & UnwrapRef<S> & _StoreWithState<Id, S, G, A> & _StoreWithGetters<G> & PiniaCustomProperties>

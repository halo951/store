export declare enum EObserverEvent {
    /** action 行为触发前 */
...
export interface IStoreObserverOptions {
     ...
     */
    [EObserverEvent.ON_COMMIT_FILTER]?: (event: ICommitFilterEvent, next: (allowChange: boolean | ICommitFilterEvent | undefined) => void) => void | Promise<void>;
    /** $patch 更新值过滤事件
     *
     ...
     */
    [EObserverEvent.ON_PATCH_FILTER]?: (event: IPatchFilterEvent, next: (allowChange: boolean | IPatchFilterEvent | undefined) => void) => void | Promise<void>;
}
/** action 行为调用前触发事件 */
...
export interface IRegisterCommitFilterOptions {
     ...
     *     - 当 next 参数为 IPatchFilterEvent 时, 将使用 `event` 中参数及值替代原始的commit.
     */
    handler: (event: ICommitFilterEvent, next: (allowChange: boolean | ICommitFilterEvent | undefined) => void) => void | Promise<void>;
}
...
export interface IRegisterPatchFilterOptions {
     ...
     *     - 当 next 参数为 IPatchFilterEvent 时, 将使用 `event` 中参数及值替代原始的commit.
     */
    handler: (event: IPatchFilterEvent, next: (allowChange: boolean | IPatchFilterEvent | undefined) => void) => void | Promise<void>;
}
export declare type TRegisterOptions = IRegisterActionBeforeOptions | IRegisterActionAfterOptions | IRegisterCommitFilterOptions | IRegisterPatchFilterOptions;
